...
function getTokenDecimals(address: string): number {
  ...
  if (addr === "0xd718019889CD2B39AD9FF2241BB17A709E980F9F".toLowerCase())
    return 6; // USDT
  if (addr === "0xe230A1eFcd14f13e5e47F45606011C65164229B3".toLowerCase())
    return 6; // USDC
  return 18;
}
...
class AegisAgent {
  ...
  private REGISTRY_ADDR =
    "0x9447878DE8F455505A17B13e9895913795f494Ed" as `0x${string}`;
  private ESCROW_ADDR =
    "0xB013B3127cdd71C1A3413FC4867F906b92dc38e4" as `0x${string}`;
  private API_BASE_URL =
    process.env.DASHBOARD_API_URL ||
    "http://localhost:3000/api/agent/reasoning";
  ...
  async reportToDashboard(
    ...
    try {
      await fetch(this.API_BASE_URL, {
        ...
        body: JSON.stringify({ mandateId, ...data }, (_, v) =>
          typeof v === "bigint" ? v.toString() : v,
        ),
      });
    } catch (err: any) {
      console.error(
        chalk.red(`[Dashboard Link] Failed to send update: ${err.message}`),
      );
    }
  }
  ...
  async initialize() {
    console.log(
      chalk.cyan("🛡️  Aegis Agent: Initializing Autonomous Service..."),
    );
    ...
    try {
      ...
      if (!isVerified) {
        console.log(
          chalk.yellow("⚠️  Agent not verified. Running in limited mode."),
        );
      }
  ...
  async processMandate(
    ...
    console.log(
      chalk.bold.cyan(`🔔 [NEW MANDATE] ID: ${attestationId.slice(0, 10)}...`),
    );
    ...
    await this.reportToDashboard(mandateId, {
      status: "analyzing",
      log: `Detected on-chain escrow of ${formattedAmount} tokens for mandate ${attestationId.slice(
        0,
        10,
      )}...`,
    });
    ...
    try {
      ...
      let goal =
        "Analyze and secure the best terms for the designated procurement mission.";
      try {
        const resp = await fetch(
          `${this.API_BASE_URL}?mandateId=${attestationId}`,
        );
        const data = (await resp.json()) as any;
        if (data.missionGoal) {
          goal = data.missionGoal;
          console.log(
            chalk.green(`[Aegis] 🎯 Directive Synchronized: "${goal}"`),
          );
        }
      } catch (fetchErr) {
        console.warn(
          chalk.dim(`[Aegis] ⚠️  Goal sync failed, using default fallback.`),
        );
      }
      ...
      await this.reportToDashboard(mandateId, {
        status: "discovery",
        log: "[Locus] 🔎 Initiating Semantic Discovery via Base Operational Layer...",
      });
      ...
      try {
        ...
        console.log(
          chalk.magenta(
            `[Locus] 🔎 Initiating Discovery via Base Operational Layer...`,
          ),
        );
        await this.reportToDashboard(mandateId, {
          status: "discovery",
          log: "[Locus] ✅ Discovery complete. Target merchant node identified and verified.",
        });
      } catch (locusErr: any) {
        console.warn(
          chalk.yellow(
            `[Locus] ⚠️  Discovery encountered an issue, proceeding with internal reasoning.`,
          ),
        );
        await this.reportToDashboard(mandateId, {
          log: "[Locus] Discovery encountered an issue, proceeding with internal reasoning.",
        });
      }
      ...
      await this.reportToDashboard(mandateId, {
        status: "reasoning",
        log: "[Venice] 🧠 Initiating Private Reasoning (Cognition Layer)...",
      });
      await new Promise((r) => setTimeout(r, 2000));
      ...
      try {
        ...
        console.error(
          chalk.red(`[Venice] ❌ AI Reasoning failed: ${aiErr.message}`),
        );
        throw aiErr;
      }
      ...
      console.log(
        chalk.magenta(
          `[Arkhai] ⚖️  Negotiation and discovery complete. Preparing settlement...`,
        ),
      );
      await this.reportToDashboard(mandateId, {
        status: "settling",
        log: "[Arkhai] ⚖️  Negotiation and discovery complete. Triggering sovereign settlement...",
      });
      await new Promise((r) => setTimeout(r, 2000));

      // Auditability: Log intent alongside financial action (Locus Requirement)
      const intentMessage = `Aegis Agent ${agent.slice(
        0,
        6,
      )} finalized procurement for mandate ${attestationId.slice(
        0,
        6,
      )} at vendor ${vendor.slice(0, 6)}. Discovery powered by Locus search.`;
      console.log(chalk.blue(`[Audit] 📝 ${intentMessage}`));

      try {
        await locusService.postIntent(attestationId, intentMessage);
        console.log(
          chalk.blue(`[Locus] ✅ Intent anchored to Locus Audit Trail.`),
        );
      } catch (auditErr) {
        console.warn(
          chalk.yellow(
            `[Locus] ⚠️  Audit logging failed, but proceeding with mission.`,
          ),
        );
      }
      ...
      console.log(
        chalk.magenta(
          `[Locus] 💳 Generating checkout for operational overhead...`,
        ),
      );
      try {
        const checkout = await locusService.createCheckout(
          0.05,
          "USDC",
          `Operational cost for Mandate ${attestationId.slice(0, 6)}`,
        );
        console.log(
          chalk.green(
            `[Locus] ✅ Checkout Link: ${checkout.url || "Generated"}`,
          ),
        );
        await this.reportToDashboard(mandateId, {
          log: `Operational checkout generated via Locus: ${
            checkout.url || "Session Active"
          }`,
        });
      } catch (payErr) {
        console.warn(
          chalk.yellow(
            `[Locus] ⚠️  Checkout generation skipped (Insufficient Credits).`,
          ),
        );
      }

      // Sign the deal
      const { message, signature } = await this.signDeal(
        vendor,
        amount.toString(),
        attestationId,
      );
      ...
      console.log(
        chalk.magenta(
          `[Celo] 🧱 Submitting mission fulfillment to Arkhai Arbiter...`,
        ),
      );
      await this.reportToDashboard(mandateId, {
      ...
      console.log(
        chalk.green(
          `[EAS] ✅ Mission Secured! Attestation linked: ${
            "0x" + keccak256(toHex(intentMessage)).slice(2, 12)
          }...`,
        ),
      );
      await this.reportToDashboard(mandateId, {
  ...
  async listen() {
    console.log(
      chalk.cyan(`📡 Aegis Reliable Listener: Active (Stateless Polling)`),
    );
      ...
      if (initialLogs.length > 0) {
        ...
        for (const log of initialLogs) {
          if (log.eventName === "FundsLocked") {
            const { attestationId, agent, vendor, token, amount } =
              log.args as any;
              ...
              console.log(
                chalk.dim(
                  `✅ Mandate ${id.slice(
                    0,
                    10,
                  )}... already settled. Syncing status.`,
                ),
              );
              this.reportToDashboard(attestationId, {
                status: "success",
                log: "Mission already completed and settled.",
              });
            } else {
            ...
            console.log(
              chalk.green(
                `🏁 Settlement detected for mandate ${attestationId.slice(
                  0,
                  10,
                )}...`,
              ),
    ...
    setInterval(() => {
      console.log(
        chalk.dim(
          `[${new Date().toLocaleTimeString()}] Aegis Heartbeat: Service Healthy`,
        ),
      );
    }, 60000);
  }
