...
import { mainnet, sepolia, baseSepolia, foundry, celo, celoSepolia } from "viem/chains";
...
function getChain(network: string) {
  switch (network.toLowerCase()) {
    ...
    default:
      throw new Error(
        `Unknown network: ${network}. Use mainnet, sepolia, base-sepolia, celo, celo-sepolia, or localhost`,
      );
  }
}
...
async function main() {
  try {
    ...
    if (!privateKey) {
      ...
      console.error("\n💡 You can either:");
      console.error(
        "   1. Set it globally: nla wallet:set --private-key <your-key>",
      );
      console.error(
        "   2. Use for this command only: --private-key <your-key>",
      );
    ...
    if (!rpcUrl) {
      if (network === "localhost") {
        ...
        console.error(
          "❌ Error: RPC URL is required for non-localhost networks. Use --rpc-url",
        );
        process.exit(1);
      }
    }
    ...
    console.log(
      `  🔑 Deployer: ${privateKey.slice(0, 6)}...${privateKey.slice(-4)}\n`,
    );
    ...
    if (balance === 0n) {
      console.error(
        "❌ Error: Deployer has no ETH. Please fund the account first.",
      );
      process.exit(1);
    }
    ...
    addresses.eas = await deployContract("EAS", EAS.abi, EAS.bytecode.object, [
    ...
    */
    ...
    const outputPath =
      args.output ||
      resolve(projectRoot, `cli/deployments/${deploymentFileName}.json`);
